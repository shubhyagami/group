...
class PreferenceRequest(BaseModel):
    ...
    behavior_tracking_enabled: Optional[bool] = None


class ProfileDto(BaseModel):
    user: UserDto
    addresses: list[AddressDto] = []
    preference: Optional[PreferenceDto] = None
...
# Search / NL Search
# --------------------------------------------------------------------------
class ParsedSearchDto(BaseModel):
    raw_query: str
...
class ProductComparisonDto(BaseModel):
    products: list[ProductCardDto]
    spec_matrix: list[CompareRow]
    identical_cells: list[int] = []
    ai_verdict: str
    provider: str
...
class ChatResponse(BaseModel):
    message: str
    conversation_id: str
    candidate_products: list[ProductCardDto] = []
...
class InteractionRequest(BaseModel):
    interaction_type: str = Field(
        pattern="^(PRODUCT_VIEW|SEARCH|ADD_TO_CART|REMOVE_FROM_CART|ADD_TO_WISHLIST|PRODUCT_PURCHASE|PRODUCT_COMPARE|FILTER_APPLY)$"
    )
...
class AdminAskResponse(BaseModel):
    answer: str
    provider: str
...
class AdminAnalyticsDto(BaseModel):
    ...
    products_with_most_negative_feedback: list[dict[str, Any]]
    top_products_by_revenue: list[dict[str, Any]]
    churn_risk_signals: list[dict[str, Any]]
    recent_orders: list[OrderDto] = []
    low_stock_products: list[ProductCardDto] = []